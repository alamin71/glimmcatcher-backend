...
// Updated: signupuser now only sends OTP and returns a token without creating the user
const signupuser = async (payload: TUser) => {
  const otp = generateOtp();
  const expiresAt = moment().add(5, 'minute');

  // Include OTP and user data in the token
  const jwtPayload = {
    ...payload,
    otp,
...
export const userServices = {
  ...
  deleteAccount,
  updatePhoneNumber,
  signupuser, // Updated to not create the user yet, just send OTP and token
};
