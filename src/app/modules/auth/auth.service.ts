...
const resetPassword = async (token: string, payload: TresetPassword) => {
  ...
  if (new Date() > user.verification?.expiresAt) {
    throw new AppError(httpStatus.FORBIDDEN, 'OTP session has expired');
  }
  ...
  // ✅ Set new plain password (will be hashed by pre-save hook)
  user.password = payload.newPassword;
  user.passwordChangedAt = new Date();
  user.verification = {
    otp: 0,
    status: false,
    expiresAt: new Date(), // Optional reset
  };
